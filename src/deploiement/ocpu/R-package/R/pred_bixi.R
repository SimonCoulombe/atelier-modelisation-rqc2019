...
bixikwargs <- function(start_date="2017-04-15 00:48", start_station_code=6079, is_member=1) {
  ...
  # duree = predict(init_objects$model, as.matrix(data_pred), s = "lambda.min")
  duree = predict(init_objects$model, as.matrix(data_pred))
  # meme_station = predict(init_objects$model, as.matrix(data_pred)) > 0.5
...
bixidata <- function(data) {
  ...
  # duree = predict(init_objects$model, as.matrix(data_pred_regression), s = "lambda.min")
  duree = predict(init_objects$model, as.matrix(data_pred))
  # meme_station = predict(init_objects$model, as.matrix(data_pred)) > 0.5
